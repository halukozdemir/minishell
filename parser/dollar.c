    ...
    {//$HOME*GJ
        ...
        if (input[i] == '$' && !in_single_quotes) // Sadece tek tırnak içinde değilsek genişletme yap
        {
            ...
            while (ft_isalnum(input[j]) || input[j] == '_')
                j++;
            key = ft_substr(input, i, j - i); // Değişken ismi al
        	value = get_env_value(jobs->env, key); // Değeri bul
            if (value)
    ...
    while (input[i])
    {
        ...
        if (input[i] == '$' && !in_single_quotes) // Sadece tek tırnak içinde değilsek genişletme yap
        {
			...
			else
			{
            	...
            	while (ft_isalnum(input[j]) || input[j] == '_')
            	    j++;
            	key = ft_substr(input, i + 1, j - i - 1); // Değişken ismi al
            	value = get_env_value(jobs->env, key); // Değerini bul
            	free(key);
            	if (value)
            	{
